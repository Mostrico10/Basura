...
class FileMoverApp:
    def __init__(self, master):
        ...
        self.archivo_seleccionado = tk.StringVar(value="")
        self.directorio_ejecutable = tk.StringVar(value="")
        self.archivo_ejecutable = tk.StringVar(value="")
        ...
        tk.Label(master, text="Archivos[separador ; ]:").place(x=20, y=110)
        self.entry_archivo = tk.Entry(master, textvariable=self.archivo_seleccionado, width=50)
        self.entry_archivo.place(x=180, y=110)
        
        tk.Label(master, text="Tiempo de espera:").place(x=20, y=140)
        self.entry_tiempo = tk.Entry(master, textvariable=self.tiempo_espera, width=10)
        self.entry_tiempo.place(x=180, y=140)
        
        # Unidad de tiempo
        self.unit_label = tk.Label(master, text="Unidad de tiempo:")
        self.unit_label.place(x=250, y=140)
        self.unit_combobox = ttk.Combobox(master, textvariable=self.unidad_tiempo, values=["segundos", "minutos", "horas"])
        self.unit_combobox.place(x=350, y=140)
        ...
        self.start_button.place(x=20, y=180)
        
        self.lock_button = tk.Button(master, text="Bloquear rutas", command=self.lock_paths)
        self.lock_button.place(x=120, y=180)
        
        self.reset_button = tk.Button(master, text="Limpiar rutas", command=self.reset_paths)
        self.reset_button.place(x=240, y=180)
        
        # Botón para ejecutar archivo .py seleccionado
        self.run_file_button = tk.Button(master, text="Ejecutar archivo .py", command=self.run_selected_file)
        self.run_file_button.place(x=350, y=180)

        # Registro de actividad
        self.log_text = tk.Text(master, width=70, height=10)
        self.log_text.place(x=20, y=220)
    ...
    def start(self):
        if self.directorio_origen.get() and self.directorio_destino.get():
            ...
            messagebox.showwarning("Advertencia", "Por favor, seleccione los directorios de origen y destino.")
    ...
    def move_files(self):
        while self.running:
            ...
            archivo_seleccionado = self.archivo_seleccionado.get()
            for archivo in archivos:
                ruta_archivo_origen = os.path.join(self.directorio_origen.get(), archivo)
                if archivo == archivo_seleccionado and os.path.isfile(ruta_archivo_origen):
                    try:
                        shutil.copy2(ruta_archivo_origen, self.directorio_destino.get())
                        print(f"El archivo '{archivo}' se ha copiado correctamente.")
                        self.log_activity(f"El archivo '{archivo}' se ha copiado correctamente.")
                    except Exception as e:
                        print(f"Error al copiar el archivo '{archivo}': {str(e)}")
                        self.log_activity(f"Error al copiar el archivo '{archivo}': {str(e)}")
    ...
    def run_selected_file(self):
        try:
            if self.archivo_ejecutable.get():
                subprocess.Popen(["python", self.archivo_ejecutable.get()])
            else:
                messagebox.showwarning("Advertencia", "Por favor, seleccione un archivo .py primero.")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo ejecutar el archivo .py: {str(e)}")
    ...
    def log_activity(self, message):
        ...
        self.log_text.see(tk.END)  # Scroll al final del texto
